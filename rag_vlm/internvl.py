...
class InternVL:
    ...
    @torch.no_grad
    def __call__(self, prompt, image_paths, history=None):

        # handle both single or multiple images
        ...
        if len(all_pixel_values) == 1:
            ...
            num_patches_list = [pixel_values.size(0) for pixel_values in all_pixel_values]
            all_pixel_values = torch.cat(all_pixel_values, dim=0)
            
        response, history = self.model.chat(
            self.tokenizer, all_pixel_values, prompt, self.generation_config,
            num_patches_list=num_patches_list, 
            history=None, return_history=True,
            )
